...
DEFAULT_PROMPT_PATH = (
    "/home/ankit/smartsense_code/fraudx_timeline_poc/workflow2/prompts/screening.md"
)
DEFAULT_PATIENT_PROFILE_PATH = (
    "/home/ankit/smartsense_code/fraudx_timeline_poc/workflow2/prompts/patient_profile.txt"
)
...
async def step2_5_screen_chunks_node(state: Dict[str, Any]) -> Dict[str, Any]:
    input_files = state.step2_outputs or []
    prompt_path = getattr(state, "prompt_step2_5_path", None) or DEFAULT_PROMPT_PATH
    patient_profile_path = (
        getattr(state, "patient_profile_step2_5_path", None)
        or DEFAULT_PATIENT_PROFILE_PATH
    )
    ...
    end_time = time.time()
    logger.info(f"Step2.5 completed in {end_time - start_time:.2f} seconds")
    return result